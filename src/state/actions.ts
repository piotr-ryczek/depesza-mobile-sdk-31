...
export const handleError =
  (error, message = '', if403ShouldLogout = true) =>
  async (dispatch) => {
...
export const followRegion =
  (regionId: string) => (dispatch, getState: () => AppState) => {
...
export const saveArticle =
  (articleId: string) => (dispatch, getState: () => AppState) => {
...
export const undoReportArticle =
  (articleId: string) => (dispatch, getState: () => AppState) => {
    const { reportedArticles } = getState();
